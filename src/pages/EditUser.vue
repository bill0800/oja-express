<template>
    <div class="container px-lg-5 px-md-3 px-sm-1">
        <div class="row pb-1">
            <span class="content-title"><i class="fa fa-list mr-3"></i> Edit User</span>
...
<script>
  ...
  import { USERS, USER } from '../router/route-names'
  ...
  export default {
    ...
    methods: {
      ...
      onEntityLoaded (response) {
        this.form.setUser(response.data.data)
      },
      onEntityUpdated (response) {
        this.form.reset()
        this.$router.push({name: USER, params: {id: response.data.data.id}})
      },
      submit () {
